...
def generate_instruction_class(
    ...
    class GenerateSingleModuleInstruction(dspy.Signature):
        ...
        proposed_instruction = dspy.OutputField(
            desc="Propose an instruction that will be used to prompt a Language Model to perform this task. In your instruction include the persona the LLM ought to act as, its goal, things it ought to always do, things it ought to never to, as well as a step by step action plan for accomplishing the goal.",
